...
#### DFA
一个 DFA M 是一个五元式: `M = (S, Σ, δ, s_0, F)`
一个 DFA 可以便是一张(确定的)状态转移图

1. S: 有限的状态的集合, 每个元素称为一个状态
2. Σ: 有穷字母表, 每个元素称为一个输入字符
3. δ: 从 S x Σ 至 S 的单值部分映射 `δ(s, a) = s', s' 是 s 的一个后继状态, `, `δ: S x Σ -> S 是一个单值函数, 唯一地确定了下一状态`
4. s_0: s_0属于S, 是唯一的初态
5. F: 终态集, F⊆S
...
### 5.3 非确定有限自动机 (NFA)
一个 NFA M 是一个五元式: `M = (S, Σ, f, S_0, F)`
非确定的体现: 初始态不唯一, 后继状态不唯一

1. S: 有限的状态的集合, 每个元素称为一个状态 (与 DFA 相同)
2. Σ: 有穷字母表, 每个元素称为一个输入字符 (与 DFA 相同)
3. f: 状态转换函数, 为 `S x Σ^* -> 2^S, f(S, α) = S', α 是字, S, S'是集合, 体现了非确定的含义`
4. S_0: S_0⊆S 是非空的初态集
5. F: 终态集, F⊆S (与 DFA 相同)

#### DFA & NFA
1. DFA 可以看作 NFA
2. DFA 和 NFA 识别能力相同 (NFA 可转换为等价的 DFA)
3. 判定两个自动机等价的算法是存在的
4. DFA 容易进行程序实现, NFA 易于人工设计
...
1. 引进新的初态结点 X, 终态结点 Y
2. 引入若干个新的状态 k, 拆分弧上的标志, 直到成为字符或 ε
3. 子集法, 解决 ε 弧与转换关系不确定
   1. 定义 I 的 ε-闭包, `J = ε-closure(I) = I ∪ {s'}`, 从某个 s ∈ I 出发经过任意条 ε 弧能到达 s'
   2. 定义 `I_a = ε-closure(J)`, J 是 某个 S ∈ I 出发经过一条 a 弧到达的状态的集合, 可以理解为 I 先经过若干条 ε 弧后在经过一条 a 弧到达的状态的集合为 I_a

#### DFA 的化简--状态的等价性
假设 s, t 为 M 的两个状态, 称 s, t 等价: 如果从状态 s 出发识别了字符串 α 且停止于终态, 从状态 t 出发识别了字符串 α 且停止于终态 (但不要求两个状态相等)

反之, 如果两个状态不等价, 则他们是可区别的

#### DFA 的化简--化简算法(最小化算法)
对一个 DFA 化简就是: 寻找一个状态数比M少的DFA M', 使得 L(M) = L(M');

换句话说: 化简的过程旨在将 M 的状态集中分割成一些不相交的子集, 使得任何两个不同子集中的状态都是可区别的, 同一子集中的任何两个状态都是等价的; 最后, 在每个子集中选取一个代表, 同时消去其他等价状态;
